      ...
      <div className="min-w-[140px]" ref={dropdownRef}>
        ...
        <div className="relative">
          <button
              ...
              aria-label={`Select branch status, current: ${selectedOption.label}`}
...
export const CreateBranchModal = ({ onClose }) => {
  ...
  const createMutation = useMutation({
    mutationFn: (data) => {
      console.log("Sending create branch request:", data);
      ...
      <Dialog.Root open={true} onOpenChange={onClose}>
        <Dialog.Portal>
          ...
          <Dialog.Content className="fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-white rounded-lg p-6 w-full max-w-md shadow-lg z-50">
            <Dialog.Title className="text-xl font-semibold text-gray-900 mb-4">Create New Branch</Dialog.Title>
            <Dialog.Description className="text-sm text-gray-600 mb-4">
              Enter details to create a new branch.
            </Dialog.Description>
            <div className="space-y-4">
              <div>
                <label className="block text-sm text-gray-700 font-medium mb-1">Branch Name</label>
                <input
                    value={formData.name}
...
export const EditBranchModal = ({ branch, onClose }) => {
  const queryClient = useQueryClient();
  ...
  const updateMutation = useMutation({
    mutationFn: (data) => {
      console.log("Sending update branch request:", data);
      return branchService.updateBranch(branch.id, data);
    },
    onSuccess: () => {
  ...
  return (
      <Dialog.Root open={true} onOpenChange={onClose}>
        <Dialog.Portal>
          <Dialog.Overlay className="fixed inset-0 bg-black/50 z-50" />
          <Dialog.Content className="fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-white rounded-lg p-6 w-full max-w-md shadow-lg z-50">
            <Dialog.Title className="text-xl font-semibold text-gray-900 mb-4">Edit Branch</Dialog.Title>
            <Dialog.Description className="text-sm text-gray-600 mb-4">
              Edit the details of the existing branch.
            </Dialog.Description>
            <div className="space-y-4">
              <div>
                <label className="block text-sm text-gray-700 font-medium mb-1">Branch Name</label>
...
export const BranchList = () => {
  ...
  const [filters, setFilters] = useState({ name: "", address: "", activeStatus: "" });
  const [searchParams, setSearchParams] = useState({ name: "", address: "", activeStatus: "" });
  const [deleteTargetId, setDeleteTargetId] = useState(null);
  const [createOpen, setCreateOpen] = useState(false);
  const [editTarget, setEditTarget] = useState(null);

  const { data, isLoading } = useQuery({
    queryKey: ["branches", page, searchParams],
    queryFn: () => {
      const cleanFilters = {
        page,
        size,
      };
      if (searchParams.name.trim()) cleanFilters.name = searchParams.name.trim();
      if (searchParams.address.trim()) cleanFilters.address = searchParams.address.trim();
      if (searchParams.activeStatus) cleanFilters.activeStatus = searchParams.activeStatus;

      return branchService.getBranches(cleanFilters);
    },
  ...
  const toggleStatusMutation = useMutation({
    mutationFn: ({ id, status }) => {
      console.log("Sending toggle status request:", { id, status });
      return branchService.changeStatus(id, status);
    },
    onSuccess: () => {
  ...
  const deleteMutation = useMutation({
    mutationFn: (ids) => {
      console.log("Sending delete branch request:", ids);
      return branchService.deleteBranches(ids);
    },
    onSuccess: () => {
  ...
  return (
      <div className="space-y-6 p-4 sm:p-6 max-w-7xl mx-auto">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <h2 className="text-2xl font-bold text-gray-900">Company Branch</h2>
          <Button
              type="button"
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-x-auto">
          <Table className="w-full">
            <TableHeader>
              <TableRow>
                <TableHead className="text-center text-gray-700 font-semibold">Branch Name</TableHead>
                <TableHead className="text-center text-gray-700 font-semibold">Address</TableHead>
              ...
              {branches.map((branch) => (
                  <TableRow key={branch.id}>
                    ...
                    <TableCell className="text-center">
                      ...
                      ${branch.activeStatus === "ACTIVE" ? "bg-green-100 text-green-700 hover:bg-green-200 ring-green-300" : "bg-red-100 text-red-700 hover:bg-red-200 ring-red-300"}
                      !rounded-full
                    `}
                          onClick={() =>
                              toggleStatusMutation.mutate({
                                id: branch.id,
                                status: branch.activeStatus === "ACTIVE" ? "INACTIVE" : "ACTIVE",
                              })
                          }
                          disabled={toggleStatusMutation.isPending}
                      >
                        {branch.activeStatus === "ACTIVE" ? "Active" : "Inactive"}
                      </Button>
                    </TableCell>
                    <TableCell className="text-center">
                      <div className="flex gap-2 justify-center">
                        <Button
                            size="sm"
                            className="bg-teal-500 hover:bg-teal-600 text-white rounded-lg"
                            onClick={() => setEditTarget({ id: branch.id, name: branch.name, address: branch.address })}
                        >
        ...
        {editTarget && <EditBranchModal branch={editTarget} onClose={() => setEditTarget(null)} />}
        {deleteTargetId !== null && (
            <ConfirmDeleteModal
                open={true}
